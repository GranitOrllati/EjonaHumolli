...
$genreNames = array('Biography', 'Autobiography');  // Replace with the actual genre names

// Create a comma-separated list of placeholders for genre names
...
$selectBiographyBooks = $con->prepare($sql);

// Bind the genre names to the placeholders
foreach ($genreNames as $index => $genreName) {
    $selectBiographyBooks->bindValue($index + 1, $genreName);
}

$selectBiographyBooks->execute();
$biographyBooks_data = $selectBiographyBooks->fetchAll(PDO::FETCH_ASSOC);
?>
...
<head>
  ...
  <body>
    ...
    <h1 class="header-h1" >Biography Books</h1>

    <div class="row">
<?php foreach ($biographyBooks_data as $book_data) { ?>
